...
import { equipmentRequests, getDashboardStats, inventoryItems } from "@/data/mockData";
import { Progress } from "@/components/ui/progress";
import { ArrowUpRight, Clock, ClipboardList, Database, AlertTriangle } from "lucide-react";
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <StatCard 
            ...
            className={stats.lowStockItems > 0 ? "border-red-200 bg-red-50" : ""}
          />
        </div>
                ...
                <div className="space-y-4">
                  {lowStockItems.map(item => (
                    <div key={item.id} className="space-y-2">
                      ...
                      <Progress value={(item.available / item.totalStock) * 100} className="h-2" />
                    </div>
                  ))}
                </div>
              )}
